...
if [[ "$FILENAME" =~ AeonOS-.*-(COMMUNITY|OFFICIAL)-.*\.zip ]]; then
...
if [[ "$FILENAME" =~ AeonOS-([0-9\.]+)_[0-9]+-$ROMTYPE-.*\.zip ]]; then
...
if [[ "$FILENAME" =~ AeonOS-.*-$ROMTYPE-(GMS|VANILLA)-.*\.zip ]]; then
    ...
    "response": [
        {
            ...
            "romtype": "$ROMTYPE",
            "size": $SIZE,
            "url": "https://github.com/AeonOS/releases/download/$VERSION/$FILENAME",
            "version": "$VERSION"
        }
    ]
}
...
echo "=========================================="
echo -e "         ${RED}Welcome to AeonOS${NC}             "
echo "=========================================="
